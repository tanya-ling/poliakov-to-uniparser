...
def moreproblems(word):
    ...
    for wordic in words:
        word0 = boary(wordic)
        words0 = normalize(words0, word0)
    words += words0''' # 09.12 гласный + йотированный вынесено в коммонформ
...
def parad_na_yer(stem):
            if re.search(u'([цкнгшщзхфвпрлджчмтб])[ое]([цкнгшщзхфвпрлджчмтб])', stem[-3:]):
                paradigm = u'N1t*/N1j*/N1t/N1j/N6t'
            elif re.search(u'[цкнгшщзхфвпрлджчмтб]', stem[-1]):
                paradigm = u'N1t/N1j/N6t'
            if stem[-2:] == u'ск':
                paradigm = u'N1k'
            if stem[-1] == u'г' or stem[-1] == u'х':
                paradigm = u'N1g'
            if re.search(u'([цкнгшщзхфвпрлджчмтб])ок', stem[-3:]):
                paradigm = u'N1k*'
            ...
            if stem[-1] == u'ц':
                paradigm = u'N1c'
            if re.search(u'([цкнгшщзхфвпрлджчмтб])ец', stem[-3:]):
                paradigm = u'N1c*'
            return paradigm
...
def parad_na_yerj(stem):
            ...
            if re.search(u'([цкнгшщзхфвпрлджчмтб])е([нфвпрлдмтб])', stem[-3:]):
                paradigm = u'N1t*/N1j*/N1t/N1j/N6t/N43*/N41/N42/N43'
            return paradigm

def parad_na_o(stem):
            if re.search(u'[цкнгшщзхфвпрлджчмтб]', stem[-1]):
                paradigm = u'N2t/N5es'
            if re.search(u'([цкнгшщзхфвпрлджчмтб])([цкнгшщзхфвпрлджчмтб])', stem):
                paradigm = u'N2t*'
...
def parad_na_e(stem):
            ...
            if stem[-1] == u'ц':
                paradigm = u'N2c'
            if re.search(u'([цкнгшщзхфвпрлджчмтб])ц', stem[-2:]):
                paradigm = u'N2c/N2c*'
...
def nonounparad(lexeme, gramm):
    ...
    if pos == u'N':
        if word[-1] == u'ъ':
            ...
            if re.search(u'[цкнгшщзхфвпрлджчмтб]', stem[-1]):
                paradigm = u'N3t'
            if re.search(u'([цкнгшщзхфвпрлджчмтб])([цкнгшщзхфвпрлджчмтб])', stem[]) #  ANTON ANTON ANTON
...
def getAllVerb(paradigm, lexeme, test, iryd, irsd):
            if test == 1:
...
def getAllParadigm_getStem(gramm, paradigm, lexeme, test, iryd, irsd):
    if lexeme == u'некто':
        print lexeme, gramm, paradigm
    if test == 1:
        # print u'getAllParadigm_getStem', lexeme, paradigm
        pass
    try:
        if gramm[0] == u'V':
...
def creating_new_dict():
    ...
    add_manually(u'./ручные лексемы/ruki.txt', nd)
    add_manually(u'./ручные лексемы/add_torot_lexemes.txt', nd)
...
def open_old_dict():
    ...
    nd = codecs.open(u'dictionary_1302.txt', u'w', u'utf-8')
    return ml, od, nd

def irrstemsopen():
    irs = codecs.open(u'.\ручные основы\irrstems_checked.txt', u'r', u'utf-8')
    irsd = {}
    for lin in irs:
        lin = lin.rstrip()
        lin = lin.split(u'	')
        irsd[lin[0]] = lin[1:]
    iry = codecs.open(u'.\ручные основы\yati_checked.txt', u'r', u'utf-8')
    iryd = {}
    for lin in iry:
        lin = lin.rstrip()
        lin = lin.split(u'	')
