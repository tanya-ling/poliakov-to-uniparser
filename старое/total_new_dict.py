...
def moreproblems(word):
    ...
    words0 = []
    for wordic in words:
        word0 = samelettter(wordic)
        words0 = normalize(words0, word0)
    words += words0

    words0 = []
    for wordic in words:
        word0 = happiness(wordic)
        words0 = normalize(words0, word0)
    words += words0
...
def verbprefixgood(wordo):
    ...
    if word[:3] == u'бес':
        ...
        word = u'бес' + word[3:]
    if word[:5] == u'чърес' or word[:6] == u'черес' or word[:6] == u'чьрес':
        word = u'через' + word[5:]
    elif word[:5] == u'через':
        word = u'черес' + word[5:] + u'.//' + u'чърес' + word[5:] + u'.//' + u'чьерес' + word[5:]
    if word != wordo:
        return word
    return

def verbbadprefix(word):
    if word[:3] == u'съг' or word[:3] == u'съз' or word[:3] == u'съв' or word[:3] == u'съд' or word[:3] == u'съж' or word[:3] == u'съб':
        word1 = u'зъ' + word[2:]
    if word[:2] == u'съ':
        word2 = u'со' + word[2:]
    elif word[:2] == u'со':
...
def open_old_dict():
    ...
    nd = codecs.open(u'total_new_dict_sevpar.txt', u'w', u'utf-8')
    return ml, od, nd
...
def od_line(ml, line, nd, irsd, iryd):
    ...
    if bealexeme == u'аала́рь':
        # print bealexeme, paradigm, u'od_line'
        test = 1
